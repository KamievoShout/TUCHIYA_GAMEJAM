...
public class FadeManager : MonoBehaviour
{
	...
	{
		StartCoroutine(FadeOutOnly(interval));
	}
	// フェードインのみ
	public void FadeIn(float interval)
	{
		StartCoroutine(FadeInOnly(interval));
	}
	...
	/// フェードアウト用コルーチン .
	/// <param name='interval'>暗転にかかる時間(秒)</param>
	private IEnumerator FadeOutOnly(float interval)
	{
		this.isFading = true;
		float time = 0;
		while (time <= interval)
	...
	/// フェードイン用コルーチン .
	/// <param name='interval'>復帰にかかる時間(秒)</param>
	private IEnumerator FadeInOnly(float interval)
	{
		this.isFading = true;
		float time = 0;
		while (time <= interval)
